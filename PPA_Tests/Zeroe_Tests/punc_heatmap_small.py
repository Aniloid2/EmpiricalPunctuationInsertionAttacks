...
if args.dataset_1 == 'QQP':
    ...
    # x_axis_labels = ['\'','-',',','.']
    # y_axis_labels = ['\'','-',',','.'] #['.',',','-','\'']
    # x_axis_labels = ['Apostrophe','Hyphen','Comma','Full Stop','Zeroe','Char' ]
    # y_axis_labels = ['Apostrophe','Hyphen','Comma','Full Stop','Zeroe','Char' ] #['.',',','-','\'']
...
fig, (ax1) = plt.subplots(1, 1, figsize=(10, 6),dpi=80, facecolor='w', edgecolor='k')
...
# sns.heatmap(ax= ax1,data=fin_list_3,norm=norm,cmap=cmap,mask=matrix, annot=True,linewidths=2,annot_kws={"size": 13},linecolor='black')
sns.heatmap(ax= ax1,data=fin_list_3,norm=norm,cmap=cmap,mask=matrix, annot=True,linewidths=0,annot_kws={"size": 25},linecolor='black')

ax1.set_xticklabels(x_axis_labels, fontsize = 20)
ax1.set_yticklabels(y_axis_labels, fontsize = 20, rotation=0)
# ax1.set_ylabel('Worst by')
ax1.set_xlabel('P = 0.8',fontsize = 25, weight='bold')#Better by
ax1.xaxis.set_tick_params(labeltop=True,labelbottom = False, bottom=False)
ax1.xaxis.set_label_position('top')
# folder_model_name_2 = folder_model_name_2.upper()
# ax1.set_title(f'{folder_data_name_2}:{folder_model_name_2}', loc='center', fontsize =17)
# folder_model_name_2 = folder_model_name_2.lower()
...
print ('wat')
fig.tight_layout()
fig.savefig(f"./Result/Non_Grammar/{folder_data_name_1}/{folder_model_name_1}_0.8_heatmap_{args.dataset_1}.png" ,bbox_inches='tight')
fig.savefig(f"./Result/Non_Grammar/{folder_data_name_1}/{folder_model_name_1}_0.8_heatmap_{args.dataset_1}.eps" ,bbox_inches='tight')
