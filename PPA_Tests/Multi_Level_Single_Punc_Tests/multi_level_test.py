...
parser.add_argument('--recipe',default="punctuation_attack",
                    help='attacked dataset')
parser.add_argument('--punc_type',default="\'-",
                    help='punc type to use')
...
def attack_setup(model_used,recipe,punctuation):
    ...
    if recipe =='punctuation_attack':
        ...
        constraints.append(input_column_modification)
        search_method = ParticleSwarmOptimization(pop_size=5, max_iters=2)
...
for d in range(len(dataset_list)):
    ...
    model_path_final = f"./{args.recipe}_{args.punc_type}_symbol/{dset}/{model_path}/Models/"
    ...
    if args.dataset_load:
        ...
        name_save = f'{model_path}-{dset}-{args.recipe}_{args.punc_type}_symbol.txt'
