...
file_name_treie = 'treie_optimal_embedding_size_'
...
print (textbugger_results, treie_results,random_false_results)
...
def after_attack_and_words(folder,results):
    ...
    for i in range(len(results)):
        ...
        after_attack.append([float(a[1]) for a in cols_treie if a[0] == 'After Attack Acc [%]:'][0])
...
# plt.plot(treie_results['query'], treie_results['attack'], color='r',marker='o', lw=2, label=f'{name}')
plt.plot(textbugger_results['query'], textbugger_results['attack'], color='b',marker='>',linestyle='dashed', lw=2, label=f'TextFooler',alpha=0.5,markersize=15)

plt.plot(treie_results['query'], treie_results['attack'], color='r',marker='o', lw=2, label=f'DWBP RPos=T/RPunc=F',alpha=0.5,markersize=15)
# for i, txt in enumerate(treie_results['query']):
#     plt.annotate(txt, (treie_results['query'][i], treie_results['attack'][i])) #color='b',marker='>', lw=2, label=f'TextFooler',alpha=0.7

# for i, txt in enumerate(textbugger_results['query']):
#     plt.annotate(txt, (textbugger_results['query'][i], textbugger_results['attack'][i]))
plt.plot(random_false_results['query'], random_false_results['attack'], color='g',marker='s', lw=2, label=f'DWBP RPos=F/RPunc=F',alpha=0.5,markersize=15)


plt.plot(all_char_false_results['query'], all_char_false_results['attack'], color='m',marker='P', lw=2, label=f'DWBP RPos=T/RPunc=T',alpha=0.5,markersize=15)

plt.plot(all_false_results['query'], all_false_results['attack'], color='k',marker='X', lw=2, label=f'DWBP RPos=F/RPunc=T',alpha=0.5,markersize=15)


# plt.xlabel('Queries',fontsize=30)
# plt.xticks([i for i in range(33,35,5)],fontsize=25)
# xmin = 32
# xmax = 35
# plt.xlim(xmin, xmax)
# # plt.yticks([i for i in range(20,35,5)],fontsize=25)
...
#
plt.xlabel('Queries',fontsize=30)
plt.xticks([i for i in range(20,100,10)],fontsize=25)
xmin = 20
xmax = 100
plt.xlim(xmin, xmax)
...
# plt.title(f'{args.dataset} with Bert-Base-Uncased \n Queries vs After attack accuracy',fontsize=40)
plt.legend(fontsize=18)
plt.savefig(f'./query_budget_{args.dataset}.png',bbox_inches='tight')
plt.savefig(f'./query_budget_{args.dataset}.eps',bbox_inches='tight')
